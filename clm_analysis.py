...
def plot_burnt_area_map_period(year_start, year_period, BA_data, grid_data, time_data):
    ...
    burnt_area = BA_data["BAF"][year_start*12:year_start*12+month_period]
    burnt_area = np.multiply(burnt_area,days_per_month[:,np.newaxis,np.newaxis]/100.)
    burnt_area = np.sum(burnt_area, axis=0)
    map_data = np.multiply(burnt_area, grid_data["cell_area"])
    # Change units.
    ...
    plt.title("Mean Burnt Area 1996-2012, CLM")
    plt.show()
...
#lot_map_period(297,16,emis_CLM,grid_CLM,time_data)    
